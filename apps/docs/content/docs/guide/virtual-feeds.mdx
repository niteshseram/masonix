...
## Restore an initial position

Use `initialScrollIndex` to restore a feed without waiting for a ref effect.

```tsx
<MasonryVirtual
  items={items}
  initialScrollIndex={{ index: savedIndex, align: 'auto' }}
  render={({ data }) => <FeedCard item={data} />}
/>
```

The initial jump runs once after the requested item is loaded. Use
`scrollRef.current.scrollToIndex()` for navigation after mount.
