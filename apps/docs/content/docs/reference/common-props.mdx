...
| `role`                     | `"list" \| "none"`                             | `"list"`       | All        | ARIA role for the outer container.                         |
| `aria-label`               | `string`                                       | -              | All        | Accessible label for the layout.                           |
| `announceItemCountChanges` | `boolean`                                      | `false`        | All        | Announces item-count changes through a polite live region. |
...
Standard HTML attributes not listed above are forwarded to the outer
container. This includes `id`, `title`, additional `aria-*` attributes,
...
Masonix owns the container children and does not accept
`dangerouslySetInnerHTML`. The `role` prop remains limited to `list` and
`none` so the generated item semantics stay valid.
