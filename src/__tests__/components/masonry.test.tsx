...
describe("Masonry", () => {
  describe("item rendering", () => {
    it("renders all items", () => {
      render(<Masonry items={["a", "b", "c", "d"]} render={Card} defaultWidth={900} columns={3} />);
      expect(screen.getAllByTestId("card")).toHaveLength(4);
    });

    it("distributes items round-robin: item i → column i % columnCount", () => {
      const { container } = render(
        <Masonry items={["a", "b", "c", "d", "e"]} render={Card} defaultWidth={900} columns={3} />,
      );
    ...
    it("passes correct index to render", () => {
      ...
      render(<Masonry items={["a", "b", "c"]} render={IndexCard} defaultWidth={900} columns={3} />);
    ...
    it("passes columnWidth as width to render", () => {
      ...
      render(<Masonry items={["a", "b", "c"]} render={Card} defaultWidth={900} columns={3} />);
  ...
  describe("effectiveColumnCount", () => {
    it("renders fewer columns when itemCount < columns", () => {
      const { container } = render(
        <Masonry items={["a", "b"]} render={Card} defaultWidth={900} columns={5} />,
      );
  ...
  describe("accessibility", () => {
    it('has role="list" by default', () => {
      const { container } = render(<Masonry items={["a"]} render={Card} defaultWidth={900} />);
    ...
    it("sets aria-label", () => {
      const { container } = render(
        <Masonry items={["a"]} render={Card} defaultWidth={900} aria-label="Photo gallery" />,
      );
  ...
  describe("gap inline style", () => {
    it("applies columnGap to container when gap > 0", () => {
      const { container } = render(
        <Masonry items={["a", "b", "c"]} render={Card} defaultWidth={900} columns={3} gap={16} />,
      );
      expect((container.firstElementChild as HTMLElement).style.columnGap).toBe("16px");
    });
    ...
    it("applies rowGap to column wrappers when gap > 0", () => {
      const { container } = render(
        <Masonry items={["a", "b"]} render={Card} defaultWidth={900} columns={2} gap={12} />,
      );
  ...
  describe("className props", () => {
    it("applies className to the container", () => {
      const { container } = render(
        <Masonry items={["a"]} render={Card} defaultWidth={900} className="my-grid" />,
      );
      expect(container.firstElementChild?.classList.contains("my-grid")).toBe(true);
    });
  ...
  describe("style prop", () => {
    it("merges user style with layout styles", () => {
      const { container } = render(
        <Masonry items={["a"]} render={Card} defaultWidth={900} style={{ background: "red" }} />,
      );
  ...
  describe("ref forwarding", () => {
    it("forwards ref to the container DOM element", () => {
      ...
      render(<Masonry items={["a"]} render={Card} defaultWidth={900} ref={ref} />);
    ...
    it("forwards ref to a custom container element", () => {
      const ref = { current: null as HTMLElement | null };
      render(<Masonry items={["a"]} render={Card} defaultWidth={900} as="ul" ref={ref} />);
